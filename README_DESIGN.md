...
本書はバックエンド中心の設計書である。実装済みフロントエンドの利用方法と画面仕様は [`README_FRONTEND.md`](./README_FRONTEND.md) を参照のこと。

---

## フロントエンド連携の現状

現在の `frontend/` 実装は、本設計の API 契約を以下の形で利用している。

- `POST /api/generate` でジョブ投入
- `GET /api/jobs/{job_id}` を可視時 2 秒、非表示時 5 秒間隔でポーリング
- `GET /api/jobs/{job_id}/audio` は `completed` 後に同一 `job_id` へ 1 回だけ実行
- `GET /api/health` は初回表示時と、アイドル時 30 秒間隔で実行
- クライアントは `status` のみで遷移し、`stage` は表示専用で扱う

この設計により、UI は backend の非同期ジョブモデルにそのまま追従する。
