# oto-factory フロントエンド設計書

## 概要

本書は `backend/` の FastAPI を利用するフロントエンドの MVP 設計を定義する。
要件は以下である。

- React ベースのフレームワークを使う
- 実装はシンプルに保つ
- ユーザーがプロンプトを入力して送信すると、バックエンドで生成が始まる
- フロントエンドはポーリングで進捗を監視する
- 完了後に MP3 をダウンロードし、自動再生を試みる
- モバイルファーストで、スマートフォン・タブレット・デスクトップを前提にする

MVP では「1ページ完結」「1ジョブずつ操作」「認証なし」を前提とする。
...
- `GET /api/health` は初期表示の補助情報として任意利用
